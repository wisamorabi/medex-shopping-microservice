...
public class PatientService {
	...
	public Status pay(int patientid, float lat, float lon) {
		...
		for (CartItem c : lst)
		{
			...
			if (pharmaceuticalDB.getPharmaceutical(c.getMedicineID()).getPrescription() == true)
				for (Prescription p : lst2)
				{
					...
					{
						if (p.getCount() > c.getCount()) found = true;
					}
				}
		...
		for (CartItem c: lst)
		{
			orderitemservice.addOrderItem(patientid, 0, new OrderItem(oinfo.getId(),c.getMedicineID(), c.getCount(), patientid, c.getPharmacyID(),  c.getName(),pharmaceuticalstockdb.getPharmaceuticalStock(c.getPharmacyID(), c.getMedicineID()).getMedicinePrice() ,0));
		}
